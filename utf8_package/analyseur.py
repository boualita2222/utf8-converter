...
from .utils    import octets_vers_hex, octets_vers_bin
...
def analyser_texte(texte):
    ...
    return {
        ...
        "nb_octets": total_oct,
        "types":     types,
        "pct_ascii": len(types[1])/len(texte)*100 if texte else 0,
    }
    ...
    for i in range(max(len(texte1), len(texte2))):
        ...
        if c1 != c2:
            diff.append({
                ...
                "c1":  c1,
                "c2":  c2,
                "h1":  octets_vers_hex(encoder_char(c1)) if c1 else "---",
                "h2":  octets_vers_hex(encoder_char(c2)) if c2 else "---",
            })
    return {
        "texte1":      texte1,
        "texte2":      texte2,
